...
$kenticoTemplatesVersion = Get-EnvVar -Key KENTICO_TEMPLATES_VERSION
...
if ($Init) {
    Remove-Item -Path ".\data\website\*" -Recurse
    ...
    if ($kenticoTemplatesVersion -eq "latest") {
        dotnet new install kentico.xperience.templates --force -v=q
    }
    else {
        dotnet new install kentico.xperience.templates::$kenticoTemplatesVersion --force -v=q
    }
    dotnet new $kenticoProjectType -n xbk --force

    $licensePath = Join-Path (Get-Location).Path ".." $licenseFileName
    Write-Host "License file path: $($licensePath)"
